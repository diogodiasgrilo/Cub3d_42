...
/*   Updated: 2023/06/20 16:16:56 by diogpere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	insert_value(t_map_errors *errors, char *line, char *check, int i)
{
	...
	while (line[i + color_or_texture] && line[i + color_or_texture] != 32)
		color_or_texture++;
	if (strncmp(line + i, check, color_or_texture) != 0)
		return (EXIT_SUCCESS);
	i += color_or_texture;
...
int	line_check(char **line, t_map_errors *errors)
{
	...
	*line = get_next_line(errors->current_fd);
	...
	while ((*line)[i] && (*line)[i] == 32)
		i++;
	if ((*line)[i] == '\n' || (*line)[i] == '\0' || 
		(strncmp(*line + i, "NO ", 3) == 0 && insert_value(errors, *line, "NO", i) != EXIT_FAILURE) ||
		(strncmp(*line + i, "SO ", 3) == 0 && insert_value(errors, *line, "SO", i) != EXIT_FAILURE) ||
		(strncmp(*line + i, "WE ", 3) == 0 && insert_value(errors, *line, "WE", i) != EXIT_FAILURE) ||
		(strncmp(*line + i, "EA ", 3) == 0 && insert_value(errors, *line, "EA", i) != EXIT_FAILURE) ||
		(strncmp(*line + i, "F ", 2) == 0 && insert_value(errors, *line, "F", i) != EXIT_FAILURE) ||
		(strncmp(*line + i, "C ", 2) == 0 && insert_value(errors, *line, "C", i) != EXIT_FAILURE))
		return (EXIT_SUCCESS);	
	else
		return (EXIT_FAILURE);
}
